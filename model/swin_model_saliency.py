...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("Inside PatchEmbed:", x.shape)
        return x, T
...
class SWINPad(nn.Module):
    ...
    def __init__(self, pretrain_img_size=224, patch_size=4, in_chans=3, embed_dim=96, depths=[2, 2, 6, 2],
                 ...
                 out_indices=(0, 1, 2, 3), frozen_stages=-1, use_checkpoint=False, pretrained_checkpoint=None, keep_patch_keys=True, load_teacher=False, num_classes=0, decoder='swin', *args,**kwargs):
        super().__init__()
        print(args,kwargs)
    ...
    def forward(self, x, B, T, W):
        ...
        for i in range(self.num_layers):
            ...
            if self.decoder_type == 'cnn':
                #x_out = x_out.view(B,-1, H, W, self.num_features[i]).permute(0, 1, 4, 2, 3).contiguous()
                #x_out = torch.mean(x_out, dim=1, keepdim=False)

                #x_out = x_out.view(-1, H, W, self.num_features[i]).permute(0, 3,1, 2).contiguous()
                x_out = x_out.view(B,-1, H, W, self.num_features[i]).permute(0, 4, 1, 2, 3).contiguous()

            x_downsample.append(x_out)
        #print(x_out.shape)
        return x_out, x_downsample
