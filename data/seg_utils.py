...
def generate_voxel_grid_V2(events, shape, nr_temporal_bins, window, separate_pol=True, normalize_event=False):
    """
    ...
    duration = window * 1000000 * nr_temporal_bins
    
    if window == 100 and nr_temporal_bins < 21:
        tau = 300000000
    else: tau = 0

    first_stamp = events[0, 2] + tau
    last_stamp = first_stamp + duration
...
def generate_voxel_grid(events, shape, nr_temporal_bins, separate_pol=True, normalize_event=False):
    ...
    # normalize the event timestamps so that they lie between 0 and num_bins
    tau = 0#300000000
    last_stamp = 2100000000 + tau #events[-1, 2] 
    first_stamp = events[0, 2] + tau#last_stamp - 300000000 #events[0, 2]#
    deltaT = last_stamp - first_stamp
    #print(f"actual: First stamp: {events[0, 2]}\t Last stamp: {events[-1, 2]}\n")
    #print(f"choosen: First stamp: {first_stamp}\t Last stamp: {last_stamp}\n")
